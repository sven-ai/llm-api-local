...
def filter_search_results(contents, threshold: float = 0.3):
    ...
    has_data = ('distances' in contents) and (len(contents['distances']) > 0)  and (len(contents['distances'][0]) > 0)
    if not has_data:
    ...
    def is_below_threshold(v): return v < threshold
    indices = [idx for idx, val in enumerate(distances) if is_below_threshold(val)]
...
def fixedChatCompletionsPairItem(x: ChatCompletionsPairItem) -> ChatCompletionsPairItem:
